...
module.exports = async (req, res) => {
    ...
    redis.hgetall(email,async function(err,data){
        if(err){
            res.status(200).json(result.success(`redis server  error  ${err}`));
            return
        }
        if(data){
            console.log(data);
            ...
            if(data.get_time !=undefined && Number(data.get_time) > nowtime - 24*60*60*1000){
                res.status(200).json(result.success("It was picked up within 24 hours, come back later"));
                return
            }
            if(data.random !=vercode){
                res.status(200).json(result.success("vercode  Invalid parameter"));
                return
            }
            if(data.status === '2'){
